(function () {
  ...
  var templatePickerMode = null;
  var otherEventsState = { activeType: null, activeColorId: null };
  ...
  function makeMarkerCircle(type, colorId) {
    var letter = type === "sick" ? "ЛН" : (type === "vacation" ? "О" : "К");
    var c = document.createElement("span");
    c.className = "cc-marker cc-marker-" + type;
    if (colorId) {
      var color = TMPL.getColor(colorId);
      if (color) c.style.background = color.bg;
    }
  ...
  function getShiftDisplayColor(event, templates) {
    if (event.colorId) {
      return TMPL.getColor(event.colorId);
    }
    var template = event.templateId ? TMPL.find(templates, event.templateId) : null;
    return template ? TMPL.getColor(template.colorId) : TMPL.getColor("maroon");
  }

  function getOtherEventDisplayColor(event) {
    if (event.colorId) return TMPL.getColor(event.colorId);
    var defaultId = DEFAULT_TYPE_COLORS[event.type];
    return defaultId ? TMPL.getColor(defaultId) : null;
  }
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      if (displayEvent) {
        ...
        if (displayEvent.type === "sick" || displayEvent.type === "vacation" || displayEvent.type === "courses") {
          body.appendChild(makeMarkerCircle(displayEvent.type, displayEvent.colorId));
        } else if (displayEvent.type === "shift") {
          var color = getShiftDisplayColor(displayEvent, templates);
  ...
  function renderUpcomingShifts() {
    ...
    upcoming.forEach(function (item) {
      ...
      var color = getShiftDisplayColor(item.event, templates);
      ...
      var displayName = item.event.eventName || item.event.templateName;
      if (displayName) {
  ...
  function updateStats() {
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      // Пропускаем дни с отпуском, ЛН или курсами
      var hasDayOff = entry.events.some(function (ev) {
        return ev.type === "vacation" || ev.type === "sick" || ev.type === "courses";
      });
  ...
  // ============ ПАЛИТРА ЦВЕТОВ СОБЫТИЯ ============

  function renderEventColorPicker(containerId, selectedColorId, onSelect) {
    ...
    TMPL.COLORS.forEach(function (color) {
      ...
      btn.type = "button";
      btn.className = "event-color-btn" + (color.id === selectedColorId ? " selected" : "");
      btn.style.background = color.bg;
      btn.setAttribute("aria-label", color.name);
      btn.addEventListener("click", function () {
        vibrate(8);
        onSelect(color.id);
        renderEventColorPicker(containerId, color.id, onSelect);
      });
  ...
  function renderShiftColorPicker() {
    var selected = editState.colorId;
    if (!selected && editState.activeTemplateId) {
      var templates = TMPL.load();
      var tpl = TMPL.find(templates, editState.activeTemplateId);
      if (tpl) selected = tpl.colorId;
    }
    renderEventColorPicker("eventColorPicker", selected, function (colorId) {
      editState.colorId = colorId;
    });
  }

  function renderOtherEventColorPicker() {
    var type = otherEventsState.activeType;
    var selected = otherEventsState.activeColorId || DEFAULT_TYPE_COLORS[type] || "green";
    renderEventColorPicker("otherEventColorPicker", selected, function (colorId) {
      otherEventsState.activeColorId = colorId;
    });
  }

  function renderBulkColorPicker() {
    var selected = bulkState.activeColorId;
    if (!selected && bulkState.selectedTemplate) {
      selected = bulkState.selectedTemplate.colorId;
    }
    if (!selected) {
      selected = DEFAULT_TYPE_COLORS[bulkState.eventType] || "maroon";
    }
    renderEventColorPicker("bulkEventColorPicker", selected, function (colorId) {
      bulkState.activeColorId = colorId;
      renderBulkCalendar();
    });
  }
  ...
  function renderEventsList() {
    ...
    events.forEach(function (ev, index) {
      ...
      if (ev.type === "shift") {
        var color = getShiftDisplayColor(ev, templates);
        row.style.borderLeftColor = color.bg;
        icon.textContent = "⏰";
        title.textContent = ev.eventName || ev.templateName || "Смена";
        ...
        var colorV = getOtherEventDisplayColor(ev);
        row.style.borderLeftColor = colorV ? colorV.bg : "#2e7d32";
        icon.textContent = "🏖️";
        title.textContent = "Отпуск";
      } else if (ev.type === "sick") {
        var colorS = getOtherEventDisplayColor(ev);
        row.style.borderLeftColor = colorS ? colorS.bg : "#e68a2e";
        icon.textContent = "📄";
        title.textContent = "Листок нетрудоспособности";
      } else if (ev.type === "courses") {
        var colorC = getOtherEventDisplayColor(ev);
        row.style.borderLeftColor = colorC ? colorC.bg : "#6a1b9a";
        icon.textContent = "🎓";
  ...
  function openEditMode(eventIndex, template) {
    ...
    if (isEditingExistingShift) {
      ...
      editState.colorId = existingEvent.colorId || null;
      editState.eventName = existingEvent.eventName || existingEvent.templateName || null;
    ...
    if (eventNameInput) {
      eventNameInput.value = editState.eventName || "";
    }
  ...
  function applyTemplateToState(tpl) {
    ...
    var eventNameInput = document.getElementById("eventNameInput");
    if (eventNameInput) eventNameInput.value = tpl.name;
  }
  ...
  function saveCurrentEvent() {
    ...
    // Читаем название из поля ввода
    var eventNameInput = document.getElementById("eventNameInput");
    var customName = eventNameInput ? eventNameInput.value.trim() : "";
    ...
    var newEvent = {
      ...
      eventName: customName || editState.activeTemplateName || null,
      colorId: editState.colorId || null,
  ...
  function openBulkModal() {
    ...
    var colorSec = document.getElementById("bulkColorSection");
    if (t) t.textContent = "Массовое добавление смен";
    if (tplSel) tplSel.hidden = false;
    if (colorSec) colorSec.hidden = false;
  ...
  function renderBulkCalendar() {
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      if (bulkState.selectedDays[key]) {
        ...
        var color = null;
        if (bulkState.activeColorId) {
          color = TMPL.getColor(bulkState.activeColorId);
        } else if (bulkState.eventType === "shift" && bulkState.selectedTemplate) {
          color = TMPL.getColor(bulkState.selectedTemplate.colorId);
        } else {
          var defaultId = DEFAULT_TYPE_COLORS[bulkState.eventType];
          if (defaultId) color = TMPL.getColor(defaultId);
        }
    ...
    keys.forEach(function (key) {
      ...
      if (bulkState.eventType === "shift") {
        ...
        scheduleData[key].events.push({
          ...
          colorId: bulkState.activeColorId || null,
          overtimeMinutes: 0
        });
      } else {
        scheduleData[key].events.push({
          type: bulkState.eventType,
          colorId: bulkState.activeColorId || null
        });
      }
      added++;
  ...
  function bindUI() {
    ...
    document.addEventListener("click", function (event) {
      ...
      if (action === "edit") {
        ...
        vibrate(15);
        SMP.confirm("Удалить это событие?").then(function (ok) {
          if (!ok) return;
          deleteEventAtIndex(idx);
        });
      }
    });
    ...
    document.addEventListener("click", function (event) {
      ...
      vibrate(10);
      otherEventsState.activeType = type;
